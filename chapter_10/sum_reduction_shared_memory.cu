...
/**
 *  Perform sum reduction with shared memory, corresponds to Fig. 10.11.
 */
__global__
void SumReductionSharedMemory(
    float* input_array,
    float* output_value
) {
    ...
    for (unsigned int stride = ceil(blockDim.x / 2.0); stride >= 1; stride = ceil(stride / 2.0)) {
        ...
        if (threadIdx.x + stride < previous_stride) {
            input_shared[index] += input_shared[index + stride];
        }
...
void runSumReduction(
    ...
    dim3 dimGrid(1);
    SumReductionSharedMemory<<<dimGrid, dimBlock>>>(input_array_d, output_value_d);
