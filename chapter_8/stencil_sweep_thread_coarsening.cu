...
/**
 *  Perform stencil sweep kernel with thread coarsening in the z direction, corresponds to Fig. 8.10.
 */
__global__
void StencilKernel(
    float* input_array,
    ...
    __shared__ float input_prev[IN_TILE_DIM][IN_TILE_DIM];
    __shared__ float input_curr[IN_TILE_DIM][IN_TILE_DIM];
    __shared__ float input_next[IN_TILE_DIM][IN_TILE_DIM];
    ...
    if (z_index_start-1 >= 0 && z_index_start-1 < size
        ...
        input_prev[threadIdx.y][threadIdx.x] = input_array[(z_index_start-1)*size*size + y_index*size + x_index];
    ...
    for (int z_index = z_index_start; z_index < z_index_start + OUT_TILE_DIM; ++z_index) {
        ...
        if (z_index+1 >= 0 && z_index+1 < size
            && y_index >= 0 && y_index < size
            && x_index >= 0 && x_index < size)
            input_next[threadIdx.y][threadIdx.x] = input_array[(z_index+1)*size*size + y_index*size + x_index];
        ...
        if (z_index >= 1 && z_index < size-1 
            ...
            if (threadIdx.y >= 1 && threadIdx.y < IN_TILE_DIM - 1
                ...
                output_array[out_index] = coefficients[0] * input_curr[threadIdx.y][threadIdx.x]
                                        ...
                                        + coefficients[5] * input_prev[threadIdx.y][threadIdx.x]
                                        + coefficients[6] * input_next[threadIdx.y][threadIdx.x];
        __syncthreads();
