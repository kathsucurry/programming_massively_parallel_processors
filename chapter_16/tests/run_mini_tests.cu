...
void test_get_accurate_predictions() {
    const uint8_t label_size = 5;
    uint8_t y[] = {1, 3, 0};
    float logits[] = {
        0.2, 0.1, 0.3, 0.2, 0.2, // Wrong prediction.
        0.1, 0.2, 0.2, 0.3, 0.2, // Correct prediction.
        0.1, 0.2, 0.2, 0.2, 0.3  // Wrong prediction.
    };
    ...
    Tensor *tensor = (Tensor *)malloc(sizeof(Tensor));
    tensor->num_dim = 2;
    tensor->dim = (uint32_t *)malloc(2 * sizeof(uint32_t));
    tensor->dim[0] = num_samples;
    tensor->dim[1] = label_size;

    float *logits_d;
    cudaMalloc((void**)&logits_d, num_samples * label_size * sizeof(float));
    cudaMemcpy(logits_d, logits, num_samples * label_size * sizeof(float), cudaMemcpyHostToDevice);
    tensor->values_d = logits_d;

    // Prepare one-hot encoding y on device memory.
    uint8_t *y_onehot = (uint8_t *)calloc(num_samples * label_size, sizeof(uint8_t));
    for (uint32_t i = 0; i < num_samples; ++i)
        y_onehot[i * label_size + y[i]] = 1;

    uint8_t *y_d;
    cudaMalloc((void**)&y_d, num_samples * label_size * sizeof(uint8_t));
    cudaMemcpy(y_d, y_onehot, num_samples * label_size * sizeof(uint8_t), cudaMemcpyHostToDevice);

    printf("Result is: %u\n", *(get_accurate_predictions(tensor, y_d)));
}
