...
void KoggeStoneExclusiveScanKernel(
    ...
    if (global_index < size && threadIdx.x > 0)
        shared_array[threadIdx.x] = input_array[global_index - 1];
    else
...
void runParallelInclusiveScan(
    ...
    KoggeStoneExclusiveScanKernel<<<dimGrid, dimBlock>>>(input_array_d, output_array_d, size);
